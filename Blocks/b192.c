...
void print128(__m128i var) {
    ...
    printf("%02X %02X %02X %02X \n %02X %02X %02X %02X \n %02X %02X %02X %02X \n %02X %02X %02X %02X \n",
           val[0], val[4], val[8], val[12], val[1], val[5], val[9], val[13], 
                         ...
                         int number_of_rounds)
{
    ...
    __m128i padding = _mm_setzero_si128();
    __m128i RIJNDAEL256_MASK_1 =
        _mm_set_epi32(0x0f0e0d0c, 0x030a0908, 0x07060504, 0x0b020100);
    __m128i RIJNDAEL256_MASK_2 =
        _mm_set_epi32(0x07060d0c, 0x0b020108, 0x0f0e0504, 0x030a0900);
    __m128i BLEND_MASK =
        _mm_set_epi32(0x00000000, 0x00000000, 0x80800000, 0x80808000);
    
    int j;

    /* loop over the data blocks */
    data1 = _mm_loadu_si128(((__m128i *)in)); /* load data block */
    data2 = _mm_loadu_si128(((__m128i *)in + 1));   printf("Data 1: ");

    data2 = (__m128i)_mm_shuffle_pd((__m128d)data2,
                                    (__m128d)padding, 0);

	__m128i keys[28];

  keys[0]= _mm_loadu_si128((__m128i*)(key));
		keys[1]=_mm_set_epi64x(0, *((long long*)(key +16)));
    for(int i=1;i<=13;i++)
	{
		keys[2*i]= _mm_loadu_si128((__m128i*)(key+(i)*24));
		keys[2*i+1]=_mm_set_epi64x(0, *((long long*)(key +(i)*24+16)));
	}
...
print128(data2);
    for (j = 1; j < number_of_rounds; j++)
    {
        ...
        128 bit blocks*/
        data1 = _mm_shuffle_epi8(data1, RIJNDAEL256_MASK_1);
        if(j==1)
        {
            printf("Shuffle: ");
            print128(data1);
        ...
        tmp2 = _mm_shuffle_epi8(tmp2, RIJNDAEL256_MASK_2);
        if(j==1)
        {
            printf("Shuffle on data2: ");
            print128(tmp2);
        }
        ...
        tmp1 = _mm_shuffle_epi8(data1, RIJNDAEL256_MASK_1);
        tmp1 = _mm_blendv_epi8(data1, data2, BLEND_MASK);
        tmp2 = _mm_blendv_epi8(data2, data1, BLEND_MASK);
        tmp2 = _mm_shuffle_epi8(tmp2, RIJNDAEL256_MASK_2);
...
int main(){
 ...
 unsigned char computed_cipher[48];
 aes128_load_key(enc_key,key);
// fgets (plain, 24, stdin);
 ...
 Rijndael192_encrypt(plain, computed_cipher, key, 12);
 for(int i=0;i<24;i++)
 {
